...
{
    $headingsfont = gt3_get_theme_option("headings_font") . gt3_get_theme_option("google_font_parameters_headings_font");

    $mainfont = gt3_get_theme_option("main_font") . gt3_get_theme_option("google_font_parameters_main_font");


    $theme_font = $mainfont.'|'.$headingsfont;

    if (is_404() || is_single()) {
	    $theme_font .= '|Damion';
    }

    $font_url = add_query_arg('family', urlencode($theme_font), "//fonts.googleapis.com/css");
...
{
    wp_enqueue_style('AllFonts', get_theme_fonts(), array());
}

add_action('wp_enqueue_scripts', 'register_theme_fonts');
add_action('admin_enqueue_scripts', 'register_theme_fonts');
