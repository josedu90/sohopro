...
if (($select_source == 'module_images' && is_array($images_ids)) || ($select_source == 'gallery_post' && $select_gallery_post !== '')) { 	
	?>
    <div class="packery_gallery_wrapper <?php echo esc_attr($custom_class); ?> personal_preloader packery_<?php echo esc_attr($uniqid) .' '. esc_attr($css_class) . ' ' . $custom_class; ?>"
	...
	<?php
		if (isset($list_array) && count($list_array) > 0) {
			...
			$compile .= '<a class="gt3_soho_button packery_load_more" href="'. esc_js("javascript:void(0)") .'"><span>' . esc_attr($button_title) . '</span></a>';
			$compile .= '</div>';
			echo (($compile));
		}
	?>
